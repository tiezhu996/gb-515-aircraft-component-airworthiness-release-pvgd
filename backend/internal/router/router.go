...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	airworthinessBlockRepository := repository.NewAirworthinessBlockRepository(db)
	airworthinessBlocker := service.NewAirworthinessBlockerService(airworthinessBlockRepository)
	aircraftPartService := service.NewAircraftPartService(aircraftPartRepository, airworthinessBlocker, securityService)
	inspectionTaskService := service.NewInspectionTaskService(inspectionTaskRepository, airworthinessBlockRepository, securityService)
	certificateRecordService := service.NewCertificateRecordService(certificateRecordRepository, securityService)
	releaseAuthorizationService := service.NewReleaseAuthorizationService(releaseAuthorizationRepository, airworthinessBlocker, securityService)
