...
type AircraftPartHandler struct{ service service.AircraftPartService }

func NewAircraftPartHandler(s service.AircraftPartService) *AircraftPartHandler {
	return &AircraftPartHandler{service: s}
}

func (h *AircraftPartHandler) Register(group *gin.RouterGroup) {
...
func (h *AircraftPartHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *AircraftPartHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *AircraftPartHandler) create(c *gin.Context) {
	var input dto.CreateAircraftPart
...
func (h *AircraftPartHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateAircraftPart
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *AircraftPartHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
