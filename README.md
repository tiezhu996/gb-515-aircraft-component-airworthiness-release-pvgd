...
| 放行授权 | `ReleaseAuthorization` | `/api/authorizations` | draft, review, approved, restricted, revoked（restricted 可在失败复检通过后重新进入 review） |
...
- 检查失败适航阻断闭环：检查任务判定失败时，同 `RelatedCode`（同编号）部件在单事务内转 `hold`、待复核/已批准授权转 `restricted`，并写入失败任务编号与原因；阻断期间部件不能放行、授权不能批准或重新提交。重新检查（`failed -> running -> passed`）通过后阻断一次性解除，部件仍保持暂停等待人工放行，授权只能重新提交复核并再次经过双人复核；重复或并发判定、恢复通过乐观锁只生效一次，失败不落半更新。部件、检查与授权页面刷新后展示同一阻断编号与原因。
