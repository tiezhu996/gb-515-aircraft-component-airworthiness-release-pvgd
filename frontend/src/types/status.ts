export interface EntityConfig {
  ...
  /** Secondary transitions shown only for the airworthiness-block closure. */
  secondaryTransitions?: Readonly<Record<string, string>>;
}
...
export const ENTITY_CONFIGS: readonly EntityConfig[] = [
	...
	{ key: 'inspectionTask', path: 'inspections', label: '检查任务', statuses: ['planned', 'running', 'passed', 'failed'] as const,
		primaryTransitions: { planned: 'running', running: 'failed', failed: 'running' },
		secondaryTransitions: { running: 'passed' } },
	...
	{ key: 'releaseAuthorization', path: 'authorizations', label: '放行授权', statuses: ['draft', 'review', 'approved', 'restricted', 'revoked'] as const,
		primaryTransitions: { draft: 'review', review: 'approved', approved: 'restricted', restricted: 'revoked' },
		secondaryTransitions: { restricted: 'review' } }
];
